...
func (lr *ListaRepository) ObterBlocosPorDataESubprefeitura(data string, subprefeitura string) ([]Bloco, error) {
	db, err := storage.OpenDatabaseConnection()
	if err != nil {
		return nil, fmt.Errorf("Erro ao obter a conexão com o banco de dados: %v", err)
	}

	rows, err := db.Query("SELECT id, nome, data, subprefeitura, local, concentracao, tamanho_id FROM blocos WHERE data = $1 AND subprefeitura ILIKE '%' || $2 || '%'", data, subprefeitura)
	if err != nil {
