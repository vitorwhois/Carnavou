...
type SubprefeituraRepository struct{}

func (lr *SubprefeituraRepository) ObterBlocosPorSubprefeitura(subprefeitura string) ([]Bloco, error) {
	db, err := storage.OpenDatabaseConnection()
	if err != nil {
		return nil, fmt.Errorf("Erro ao obter a conexão com o banco de dados: %v", err)
	}

	rows, err := db.Query("SELECT id, nome, data, subprefeitura, local, concentracao, tamanho_id FROM blocos WHERE subprefeitura ILIKE '%' || $1 || '%'", subprefeitura)
	if err != nil {
