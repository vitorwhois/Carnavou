...
type BlocoRepository struct{}

func (lr *BlocoRepository) ObterBlocosPorNome(nome string) ([]Bloco, error) {
	db, err := storage.OpenDatabaseConnection()
	if err != nil {
		return nil, fmt.Errorf("Erro ao obter a conexão com o banco de dados: %v", err)
	}

	rows, err := db.Query("SELECT id, nome, data, subprefeitura, local, concentracao, tamanho_id FROM blocos WHERE nome ILIKE '%' || $1 || '%'", nome)
	if err != nil {
